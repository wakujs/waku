...
refetch(
  rscPath: string,
  rscParams?: unknown,
  enhanceFetchRscStore?: (store) => store,
)
```
...
- The third argument is an advanced escape hatch for custom fetch-store behavior and is usually unnecessary unless you are building router-like abstractions.
...
- [../examples/31_minimal](Minimal SSR and prerendered root route)
- [../examples/33_promise](Using `Children` to compose client content into a server tree)
- [../examples/34_functions](Server functions returning `_value` and updated elements)
- [../examples/35_nesting](Nested slots, `useRefetch`, `withRequest`, and fallback HTML generation)
- [../examples/36_form](Server actions and `formState`)
- [../examples/39_api](Custom API endpoints alongside minimal SSR)
- [../examples/51_spa](SPA-style build using `generateDefaultHtml`)
