...
Every component in a Waku app is a server component unless marked otherwise. Server components execute on the server, at build time if the route is static or on each request if it is dynamic, and only their rendered output is sent to the browser. Their code, and any server-only modules they import, never becomes part of the client bundle.
