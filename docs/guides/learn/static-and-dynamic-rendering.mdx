...
Waku does not implicitly cache dynamic rendering or the data you fetch inside it. What you fetch is what you render, so there is no revalidation step when your data changes; the next request is simply fresh. If you ever want caching, you add it explicitly around the specific expensive work, as a deliberate optimization. The full reference for these layers, including the caches that can exist around a Waku app, is [Rendering and Freshness](/guides/rendering-and-freshness).
