...
Components are server components unless you say otherwise. They execute on the server, can be `async`, and can fetch data, read files, or query databases directly. Their output, not their code, is sent to the browser. When you need interactivity, you add `'use client'` to a file, and that component (plus everything it imports) becomes a client component running in the browser.
