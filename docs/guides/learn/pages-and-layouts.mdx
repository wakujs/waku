...
- The `<title>` element is hoisted into the document head automatically. The same works for `meta` and `link` tags. When a layout and a page both declare a `title`, `description`, or `og:*` tag, the page's wins — see [Metadata](/guides/metadata).
