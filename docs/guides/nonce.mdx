---
slug: nonce
title: Configure nonce CSP
description: Configure nonce Content Security Policy for inline script.
---
...
Set up a middleware in `src/middleware/nonce.ts`:
...
In the case of SSG, a nonce is not secure because it must be randomly generated for every response. Since SSG produces static HTML, this is not possible. If your security requirements prioritize a nonce over static HTML benefits, consider disabling HTML pre-rendering.

(Will we explore any alternative for case of SSG?)
