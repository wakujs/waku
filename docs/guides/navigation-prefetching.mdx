...
A redirect that leaves the app hands the page to the browser, and reports `error` for the navigation it was asked to make, because that navigation never happened. The full page load follows. A redirect to a route of this app reports `complete` on the destination instead, since the server answers with it.
