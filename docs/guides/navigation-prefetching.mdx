...
It waits for a 404 page that the response leads to, but not for a redirect thrown while your page renders. It also does not wait for React to finish rendering the destination, so the address bar may still show the previous URL when it resolves.
