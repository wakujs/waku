...
The promise also rejects when the navigation fails outright, and when a redirect thrown while rendering hands the page to the browser. A redirect the server resolves before rendering starts is carried by the response instead, and there the promise resolves; the browser leaves once that response lands. Catch it if you call these programmatically, or an auth redirect at the fetch layer will surface as an unhandled rejection. It does not wait for React to finish rendering the destination, so the address bar may still show the previous URL when it resolves.
...
A redirect that leaves the app hands the page to the browser. One thrown while rendering reports `error` for the navigation it was asked to make, and the full page load follows. One the server resolves before rendering starts arrives in the response instead, so the navigation reports `complete` and the browser leaves after it.
