...
The effect runs for the initial route and whenever the current route fields change. It observes the current route, not the start or failure of a navigation. Programmatic `push`, `replace`, and `reload` calls return promises, so handle their completion or errors at the call site when needed.
