...
`router.prefetch()` accepts the same targets as `router.push()`: a typed route href (autocompleted from your routes), or a structured `{ to, params }` target for prefetching a dynamic route.
