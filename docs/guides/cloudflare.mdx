...
Create your project with `npm create waku@latest -- --template 07_cloudflare` to use the Cloudflare Workers starter template. Then use these commands:
...
To build a Waku app for Cloudflare Workers, simply set the `CLOUDFLARE` or `WORKERS_CI` environment variable to `1` and run `waku build`. Waku will use the default Cloudflare Workers adapter. You can also use a `waku.server.ts` that imports from 'waku/adapters/cloudflare'.
...
> The Waku starter template for Cloudflare Workers includes includes an example `./src/waku.server.ts` file and sets the **`WORKERS_CI`** environment variable in the build script in `package.json`:

```json
{
  "scripts": {
    "build": "WORKERS_CI=1 waku build"
  }
}
```
...
To test your build, you can add Cloudflare's CLI tool [`wrangler`](https://www.npmjs.com/package/wrangler) as a development dependencies:
...
Import from `cloudflare:workers` to access Cloudflare Workers bindings such as D1 databases, KV namespaces, Durable Objects, and more. See [Cloudflare's documentation on Workers bindings](https://developers.cloudflare.com/workers/runtime-apis/bindings/) for more information.
...
A configuration in the `wrangler.jsonc` file tells Cloudflare to route requests to that assets folder first and then fallback to handle the request with the worker.
