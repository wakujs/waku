...
To build a Waku app for Cloudflare Workers, simply set the `CLOUDFLARE` or `WORKERS_CI` environment variable to "1" and run `waku build`. Or you can use a `waku.server.ts` that imports from 'waku/adapters/cloudflare'.
