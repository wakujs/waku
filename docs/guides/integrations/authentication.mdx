...
Put the check in the layout that wraps the protected routes. A layout runs for every route beneath it, on both a full page load and a client-side navigation:
...
Server actions and API routes are not covered by a layout, so each one checks for itself. An action is an endpoint; nothing secures it but its own body:
