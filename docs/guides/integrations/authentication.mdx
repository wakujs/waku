...
A visitor with no session who clicks a `<Link to="/dashboard">` gets the page. The check passes the obvious manual test (loading the URL redirects) and fails on the path users actually take.

That much is fixable. `unstable_parseRouterRequest` reads either url as the route it addresses, so both lines above report `/dashboard` and a middleware written against it matches both — the [redirect maps](/guides/redirect-maps) guide builds one:

```ts
import { unstable_parseRouterRequest as parseRouterRequest } from 'waku/router/server';
