...
A dynamic route can opt in through `getConfig`'s `unstable_getEtag`, and this is where waku-cache fits: `cache.rsc` returns a `getEtag` that hashes the cached subtree, so the tag stays stable while the entry is fresh and changes exactly on invalidation or TTL expiry:
