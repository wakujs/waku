...
with an untouched, streamable body. Calling `input.fn()` on a marked request
whose form data contains no server action reference resolves without form
state, so the page renders normally — a plain HTML form can inherit the
marker from the document URL after a no-JS action submission.
