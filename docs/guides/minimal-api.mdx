...
- `Root` is required for `Slot`, `Children`, `useMergeElements`,
  `useRegisterRscEnhancer`, and `useRegisterRscReloadListener`. Outside a
  `Root`, `useFetchRsc` still works but runs no enhancers.
...
- `Root` injects a `generator` meta tag. Charset and viewport are yours to
  declare.
...
- A higher `order`, the optional second argument that defaults to `0`, wraps a lower one, so it runs earlier on the request and later on the result. At the same order a later registration wraps an earlier one. The Waku router registers its own enhancer at order 100. An enhancer above that order must not delay the result it returns. The router acts on an action's result while the chain is still unwinding: it applies the route change and decides whether a rerender of the page the action came from still applies. The elements arrive only once the whole chain resolves, so a delay above the router can paint a route whose slot has not been merged, and a navigation that starts during the delay can be undone when the elements arrive, even back to the page the user left.
