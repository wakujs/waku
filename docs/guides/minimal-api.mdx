...
| `decodeAction(formData)`                    | Decode a no-JS server action submission from multipart form data; resolves the action to invoke, or `null` when the form data contains no server action reference. Validates the request origin. |
| `decodeFormState(result, formData)`         | Derive the `useActionState` form state from an action result, for passing to `renderHtml`.                                                                                                       |
...
A form submitted without JavaScript arrives as an ordinary multipart `POST`
on the `custom` path, with an untouched, streamable body. When your handler
decides the request is a form submission, decode it with the utils:
`decodeAction` resolves the server action to invoke, or `null` when the
form data contains no server action reference (a plain HTML form or a
crawler), and `decodeFormState` derives the `useActionState` state from the
action result. Render the response directly, as in traditional HTTP: the
submission stays a single round trip and the returned state renders without
JavaScript, matching React's behavior. `useActionState` permalinks need no
special handling.
...
if (input.type === 'custom' && input.pathname === '/') {
  ...
  if (
    ...
    input.req.headers.get('content-type')?.startsWith('multipart/form-data')
  ) {
    ...
    if (action) {
      ...
      // an ordinary multipart form: treat it like any custom POST
