...
contains no server action reference. Respond to `undefined` with a `303`
redirect to the URL without the marker (Post/Redirect/Get): the marker then
never persists in the document URL, so plain forms on the page keep
submitting unmarked and arrive as `custom`. Only render the POST response
directly when there is form state to deliver (`useActionState`).
...
if (
  ...
  if (input.type === 'action' && formState == null) {
