...
with an untouched, streamable body. `input.fn()` resolves the decoded
form state, or `undefined` when the action produced none or the form data
contains no server action reference. Always respond with a `303` redirect to
the URL without the marker (Post/Redirect/Get): the marker then never
persists in the document URL, so plain forms on the page keep submitting
unmarked and arrive as `custom`, and a refresh never resubmits the action.
As a consequence, `useActionState` form state is delivered to clients with
JavaScript (via the function call path), not to no-JS submissions — the
action itself still runs.

When passing the `permalink` argument to `useActionState`, include the
marker in the URL, because React replaces the generated form target with
the permalink string verbatim:
...
The no-JS submission then posts to the permalink, runs the action, and the
`303` redirect lands on the clean URL (`/account`).

Statically prerendered pages bake a relative `?__waku_action=1` form target
at build time, and a query-only relative reference replaces the document's
query — so the action request itself does not carry runtime search params
(`/catalog?page=2` posts to `/catalog?__waku_action=1`). The router restores
them on the redirect from the same-origin, same-path `Referer` header, so
the user lands back on `/catalog?page=2` (a Minimal API handler can mirror
this by reading the `Referer` header before building the redirect). Clients that suppress the referrer
land on the plain path instead, and server actions on static pages should
not rely on reading runtime search params from the request URL.
...
`action` requests run the decoded server action and respond with a redirect
(see the progressive enhancement section below):
