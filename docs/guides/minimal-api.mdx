...
| `waku/minimal/client` | `unstable_*` fetch-store helpers | Building router-like abstractions.                                    |

The components and hooks without `unstable_` names are the intended minimal client primitives. This guide does not document every exported helper from `waku/minimal/client`; undocumented exports are for Waku internals or custom integrations and should be treated as unstable.
...
Wrap the body of `handleRequest` in `unstable_runWithRequest(input.req, ...)`
(from `waku/internals`). This establishes the request context for the render, so
that `unstable_getRequest()` and `unstable_getHeaders()` work in your components.
You can nest your own `AsyncLocalStorage` inside it as well.
...
handleBuild: async ({ renderRsc, generateFile, rscPath2pathname }) => {
  await runWithRequest(new Request('http://localhost:3000/'), async () => {
...
`runWithRequest(...)` matters when your render path reads request-scoped data such as the URL, cookies, or values stored in request context. At build time there is no real request, so pass a synthetic `Request`.
...
- [Minimal SSR and prerendered root route](https://github.com/wakujs/waku/tree/main/examples/31_minimal)
- [Using `Children` to compose client content into a server tree](https://github.com/wakujs/waku/tree/main/examples/33_promise)
- [Server functions returning a value and updated elements](https://github.com/wakujs/waku/tree/main/examples/34_functions)
- [Nested slots, `useRefetch`, `runWithRequest`, and fallback HTML generation](https://github.com/wakujs/waku/tree/main/examples/35_nesting)
- [Server actions and `formState`](https://github.com/wakujs/waku/tree/main/examples/36_form)
- [Custom API endpoints alongside minimal SSR](https://github.com/wakujs/waku/tree/main/examples/39_api)
- [SPA-style build using `generateDefaultHtml`](https://github.com/wakujs/waku/tree/main/examples/51_spa)
