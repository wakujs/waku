...
`action` is detected without reading the request body: waku appends a
`__waku_action=1` query parameter to the form target of every progressively
enhanced server-action form it renders, and only multipart `POST` requests
carrying that parameter are classified as `action`. Any other `POST` —
including plain HTML forms and requests from crawlers — arrives as `custom`
with an untouched, streamable body. `input.fn()` consumes the request body
and resolves the decoded form state, or `undefined` when the action produced
none or the form data contains no server action reference.

Render the POST response directly with the form state, as in traditional
HTTP: the submission stays a single round trip and `useActionState` shows
the returned state without JavaScript, matching React's behavior.

The marker then stays in the document URL after a no-JS submission. A plain
multipart form without an explicit `action` on that document inherits it and
is classified as `action`; its form data contains no server action
reference, so `input.fn()` resolves `undefined` and the body is not
delivered. Give such forms an explicit unmarked `action` to keep them on the
`custom` path.

When passing the `permalink` argument to `useActionState`, wrap it with
`unstable_patchPermalink` from `waku/client`, because React replaces the
generated form target with the permalink string verbatim and the marker
must be part of it:
...
const [state, formAction] = useActionState(
  update,
  initial,
  patchPermalink('/account'),
);
```

The no-JS submission then posts to the permalink and the response renders
there with the returned state, which is React's MPA form flow.

Statically prerendered pages do not participate in no-JS server actions:
there is no request URL to derive a marked form target from, so the encoder
opts out and React renders its client-only fallback. Submissions made before
hydration are queued and replayed once JavaScript loads (with full form
state semantics), and with JavaScript disabled the form is inert. Pages that
need no-JS server actions should be rendered dynamically.
...
`action` requests run the decoded server action and render HTML with the
form state (see the progressive enhancement section below):
...
| `parseRsc(stream)`                          | Parse an RSC stream back into an elements object. Useful when you need to inspect an RSC payload rather than stream it directly. |
...
| `parseRsc(stream)`                          | Parse an RSC stream back into elements. Useful for build-time inspection.                          |
...
Forms submit without JavaScript for actions used directly
(`<form action={fn}>`), bound in server components (`fn.bind(null, arg)`), and
bound in client components. One combination is limited: when the same action
appears on a page both unbound and bound in a client component, the no-JS
fallback for the bound form may drop its bound arguments (waku logs a warning
during SSR when this happens). Bind arguments in a server component or pass
them as hidden form fields instead. This follows from a React limitation and
resolves automatically once React provides a stable identity to custom
form-action encoders.
