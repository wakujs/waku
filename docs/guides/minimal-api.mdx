...
Always respond with a `303` redirect to the URL without the marker
(Post/Redirect/Get). The marker then never persists in the document URL, so
plain forms on the page keep submitting unmarked and arrive as `custom`, and
a refresh never resubmits the action.
