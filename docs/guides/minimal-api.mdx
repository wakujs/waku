...
| ------------ | ---------------------------------------------------------------------------------------- | ---------------------- |
| `rsc`        | RSC payload fetches initiated by `Root` or `unstable_fetchRsc`                           | `rscPath`, `rscParams` |
...
- Each `unstable_fetchRsc` call issues a new request. A router-like abstraction can own prefetching and response reuse.
...
- `Root` is required for `Slot`, `Children`, and `useMergeElements_UNSTABLE`.
...
The deprecated `useRefetch` export remains temporarily for migration. New code should define the hook locally as above.
