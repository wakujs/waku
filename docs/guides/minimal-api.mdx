...
- Throwing from `handleRequest` turns into an HTTP error response, and a custom error keeps its status. A location is a redirect a document request answers with a 3xx, but a fetch cannot follow one it is unable to read, so an RSC or server function request answers 200 and the client throws an error carrying `unstable_documentLocation` for you to navigate to.
