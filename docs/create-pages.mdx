...
A handler interceptor wraps each render, in both the request and build phases, so it runs inside the request scope where `unstable_getRequest` is available. Use it to install an `AsyncLocalStorage` for the render or to set the inline-script nonce.

```ts
type HandlerInterceptor = <T>(next: () => Promise<T>) => Promise<T>;
```

Register interceptors with `createInterceptor` inside `createPages`. They compose as onion wrappers around `next`, in registration order.
...
The `HandlerInterceptor` type is available from `waku/router/server`. The file-system router has an equivalent `src/pages/_interceptors/` convention.
