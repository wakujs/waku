...
`lazy` Slice Components are a new addition with the Slices API. They are waku's take on the [server islands feature](https://astro.build/blog/astro-4120/) from Astro. `lazy` allows components to be requested indepent from the page they are used on. This is useful for components that are expensive to render or that could be hosted separately (e.g. on a CDN).
