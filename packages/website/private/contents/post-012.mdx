...
Slices are components that are defined in the `src/pages/slices` directory. This allows us to re-use slices across pages as any other component would be used.
...
By default, the slice id will be the filename. If slices are nested like `src/pages/slices/one/two.tsx`, the id will be `one/two`.

An example for `src/pages/slices/one.tsx`:
